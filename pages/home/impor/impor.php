...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
        ...
        $sqlDelete = "DELETE FROM data_mahasiswa";
        if ($conn->query($sqlDelete) === FALSE) {
            echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to delete existing data: " . $conn->error . "',
        ...
        $sqlInsert = $conn->prepare("INSERT INTO data_mahasiswa (nama, nim, ipk_1, ipk_2, ipk_3, ipk_4, ipk_5, ipk_6, ipk_7, ipk_8, prediksi_svm, prediksi_mlp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
